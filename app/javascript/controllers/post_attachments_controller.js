...
export default class extends Controller {
    ...
    connect() {
        ...
        this.slides = this.hasTrackTarget
            ? Array.from(this.trackTarget.querySelectorAll(":scope > .feed-post-card__media-slide"))
            : [];
        ...
        if (this.hasIndicatorsTarget) {
            if (!many) {
                ...
                this.indicatorsTarget.innerHTML = Array.from({ length: this.count })
                    .map(
                        (_p, i) =>
                            `<button type="button" class="feed-post-card__indicator-dot${i === this.index ? " is-active" : ""
